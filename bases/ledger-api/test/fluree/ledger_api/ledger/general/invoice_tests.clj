(ns fluree.ledger-api.ledger.general.invoice-tests
...
(deftest add-schema
  (testing "Add schema for the invoice app"
    (let [txn         (edn/read-string (slurp (io/resource "invoice/schema.edn")))
...
(deftest add-sample-data
  (testing "Add sample data for the invoice app"
    (let [txn       (edn/read-string (slurp (io/resource "invoice/data.edn")))
