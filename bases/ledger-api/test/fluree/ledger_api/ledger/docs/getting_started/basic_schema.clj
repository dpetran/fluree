(ns fluree.ledger-api.ledger.docs.getting-started.basic-schema
...
(deftest add-predicates
  (testing "Add predicates for the chat app"
    (let [
          predicate-txn   (edn/read-string (slurp (io/resource "ChatApp/chatPreds.edn")))
...
(deftest add-sample-data
  (testing "Add sample data for the chat app"
    (let [predicate-txn   (edn/read-string (slurp (io/resource "ChatApp/chatAppData.edn")))
          collection-resp (async/<!! (fdb/transact-async (get-conn) test/ledger-chat predicate-txn))]
