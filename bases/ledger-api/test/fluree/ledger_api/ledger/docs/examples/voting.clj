(ns fluree.ledger-api.ledger.docs.examples.voting
...
(deftest add-schema
  ...
  (let [txn         (edn/read-string (slurp (io/resource "Voting/schema.edn")))
...
(deftest add-sample-data
  ...
  (let [txn       (edn/read-string (slurp (io/resource "Voting/data.edn")))
...
(deftest add-permissions
  ...
  (let [txn       (edn/read-string (slurp (io/resource "Voting/permissions.edn")))
        data-resp (async/<!! (fdb/transact-async (basic/get-conn) test/ledger-voting txn))]
