(ns fluree.ledger-api.ledger.api.downloaded
...
(deftest add-schema*
  (testing "Add schema"
    (let [tx            (edn/read-string (slurp (io/resource "ChatAltVersion/schema.edn")))
          schema-res    @(http/post (str endpoint-url "transact") (standard-request tx))
          status        (:status schema-res)
          _ (println (pr-str schema-res))
...
(comment
  @(http/post (str "http://localhost:8090/fdb/" "dbs")
              {:headers {"content-type" "application/json"} })
  {:opts {:headers {"content-type" "application/json"}, :method :post, :url "http://localhost:8090/fdb/dbs"}, :body "[[\"dan\",\"test1\"]]", :headers {:content-length "17", :content-type "application/json; charset=utf-8", :date "Sat, 15 Jan 2022 04:36:58 GMT", :server "http-kit", :x-fdb-version "polylith"}, :status 200}
...
(deftest new-people-comments-chats-auth
  (testing "Add auth, roles, rules to test.two"
    (let [tx            (edn/read-string (slurp (io/resource "ChatAltVersion/people-comments-chats-auth.edn")))
