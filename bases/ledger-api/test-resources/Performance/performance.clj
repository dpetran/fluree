(ns fluree.ledger-api.ledger.Performance.performance
...
(defn performance-check
  ...
  ([conn dbid]
    (performance-check conn dbid "Performance/QueryTxnList.edn" 0 true))
  ([conn dbid queryTxnFile offset schema?]
    (let [add-schema-res         (when schema? (add-schema-performance-check conn dbid))
          ...
          queries                (-> queryTxnFile io/resource slurp edn/read-string)
...
(comment
  ...
  (def queries (-> "Performance/QueryTxnList.edn" io/resource slurp edn/read-string))
  ...
  (def myPerformanceCheck (performance-check conn "plane/demo" "Performance/PlaneQueryList.edn" 1000
                                             false))
  ...
  (def res1 (-> "performanceMetrics/0-10-4.edn" io/resource slurp edn/read-string))
  (def res2 (-> "performanceMetrics/0-11-0-old-version.edn" io/resource slurp edn/read-string))
  ...
  (count (keys (-> "Performance/QueryTxnList.edn" io/resource slurp edn/read-string)))

  (def queryTxns (-> "Performance/QueryTxnList.edn" io/resource slurp edn/read-string))
